...
def figure_expected_grade_vs_score(
    scores: np.ndarray, out: Dict, cfg: GraderConfig,
) -> plt.Figure:
    """Figure 4: Expected grade versus score with cubic fit by default."""
    _setup_style()

    scores = np.asarray(scores, dtype=float)
    exp_grade = np.asarray(out["expected_grade"], dtype=float)
    gran_lab = np.asarray(nearest_granulated_label(exp_grade), dtype=str)
    colors = [GRADE_COLORS.get(g, "#808080") for g in gran_lab]
    ...
    ax.scatter(x, y, c=c, s=95, edgecolors="black", linewidths=1.0, zorder=3)
    ...
    if deg >= 1:
        ...
        xgrid = np.linspace(float(np.min(x)), float(np.max(x)), 250)
        ax.plot(xgrid, poly(xgrid), "k--", linewidth=2.0, alpha=0.55,
                label=("Cubic fit" if deg == 3 else "Linear fit"))

    for thresh, label in zip(GRANULATED_GRID, GRANULATED_LABELS):
        ax.axhline(thresh, color="gray", linestyle=":", alpha=0.28, linewidth=0.8)
        ax.text(float(np.max(x)) + 1.5, thresh, label, ha="left", va="center",
                fontsize=8, color="gray")

    ax.set_xlabel("Score")
    ax.set_ylabel("Expected grade value")
    ax.set_title("Expected grade versus score")
    ax.set_xlim(max(0.0, float(np.min(x)) - 5.0), min(100.0, float(np.max(x)) + 8.0))
    ax.set_ylim(-0.2, 4.2)
